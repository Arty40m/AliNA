...
class FastaRead():
    ...
    def iter_func(self):
        ...
        for l in self.file:
            ...
            if l[0] == '>':
                if name is not None:
                    raise ValueError(f'Name "{name}" without sequence. (Note that all sequence names must start with ">" symbol)')
                name = l[1:-1]
            
            else:
                if name is None:
                    raise ValueError(f'Sequence without name "{l[:-1]}". (Note that all sequence names must start with ">" symbol)')
